// Forked and modified from https://github.com/getsentry/sentry-javascript-bundler-plugins/blob/fd2a99c4ffb79974db1830ab59cd59a32433f60c/packages/bundler-plugin-core/src/debug-id-upload.ts#L39.
  ...
  return async (buildArtifactPaths: string[]) => {
    ...
    try {
      const tmpUploadFolder = await fs.promises.mkdtemp(
        path.join(os.tmpdir(), "decipher-bundler-plugin-upload-")
      );
...
export async function prepareBundleForDebugIdUpload(
  ...
  if (debugId === undefined) {
    logger.debug(
      `Could not determine debug ID from bundle. This can happen if you did not clean your output folder before installing the Decipher plugin. File will not be source mapped: ${bundleFilePath}`
    );
    return;
  }
...
function determineDebugIdFromBundleSource(code: string): string | undefined {
  const match = code.match(
    /decipher-dbid-([0-9a-fA-F]{8}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{12})/
  );
...
async function determineSourceMapPathFromBundle(
  ...
  const sourceMappingUrlMatch = bundleSource.match(
  ...
  if (sourceMappingUrlMatch) {
    ...
    if (isSupportedUrl) {
      ...
      absoluteSourceMapPath = path.join(
...
async function prepareSourceMapForDebugIdUpload(
  ...
  try {
    ...
    logger.error(
      `Failed to read source map for debug ID upload: ${sourceMapPath}`,
      e
    );
    return;
  }
  ...
  try {
    map = JSON.parse(sourceMapFileContent) as {
      sources: unknown;
      [key: string]: unknown;
    };
    ...
    logger.error(
      `Failed to parse source map for debug ID upload: ${sourceMapPath}`
    );
    return;
  }
  ...
  try {
    ...
    logger.error(
      `Failed to prepare source map for debug ID upload: ${sourceMapPath}`,
      e
    );
    return;
  }
